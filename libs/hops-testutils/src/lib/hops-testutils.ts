...
export function testFhirQuestionnaire(id: string) {
    ...
    questionnaire.id = id;
    let questionnaireResponse = createFhirQuestionnaireResponse(questionnaire, practitioner, patient, answers);
...
export async function pushResource(fhirServerUrl: URL, resource: IResourceList): Promise<IResourceList> {
