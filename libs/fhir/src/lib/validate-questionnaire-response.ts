...
function validateQuestionnaireResponse(
  ...
  if (!questionnaireResponse.questionnaire?.startsWith('Questionnaire'))
    errors.push(
      'QuestionnaireResponse `questionnaire` need to be a canonical reference'
    );
  if (
  ...
  if (questionnaireResponse.subject?.type !== 'Patient')
    errors.push(
      'QuestionnaireResponse `subject` need to be a logical reference to Patient'
    );
  if (
    ![OID_FNR, OID_DNR].includes(
      questionnaireResponse.subject?.identifier?.system
    )
  )
    errors.push(
      'QuestionnaireResponse `subject` need to be using norwegian FNR or DNR.'
    );
  if (
    ![OID_FNR, OID_HPR_NR].includes(
      questionnaireResponse.source?.identifier?.system
    )
  )
    errors.push(
      'QuestionnaireResponse `source` need to be using Norwegian FNR or HPR NR.'
    );
  return errors;
}
