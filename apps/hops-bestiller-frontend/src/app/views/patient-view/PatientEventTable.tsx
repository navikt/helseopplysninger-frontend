...
const PatientEventTable: React.FunctionComponent<Props> = ({events}) => {
        ...
        events.forEach(event => {
            rader.push({
                celler: [
                    ...
                    <div onClick={goto.viewPath("event", event.id)}><HoyreChevron style={{float: "right"}}/></div>
                ]
            })
        });
        ...
        return (
            <>
                ...
                />
                <br/>
                <Hovedknapp className={"knapp-bestill-helseopplysninger"} onClick={goto.viewPath("order")}>Bestill
