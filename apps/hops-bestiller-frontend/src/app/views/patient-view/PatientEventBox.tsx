  ...
  return (
    <div className={'patient-event-box'}>
      <div style={{ float: 'right' }} className={'patient-event-box-navigation'}>
        <span
          className={classNames({ clickable: prevEventId })}
          onClick={goto.viewPath('event', prevEventId)}
        >
          Forrige
        </span>
        <span
          className={classNames({ clickable: nextEventId })}
          onClick={goto.viewPath('event', nextEventId)}
        >
          Neste
        </span>
      </div>
      ...
      <hr />
      <br />
      <Button variant={'primary'}>Send Melding</Button>
      <br />
    </div>
  );
};
